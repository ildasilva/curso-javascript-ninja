(function(win, doc){
  ...
  */
  ...
  `$inputUsername`.
  */
  let $inputUserName = doc.querySelectorAll('input[type="text"]');
  /*
  - Selecione o input de "Email", atribuindo-o à uma variável chamada
  `$inputEmail`.
  */
  let $inputEmail = doc.querySelectorAll('input[type="email"]');
  /*
  - Selecione o campo de "Mensagem", atribuindo-o à uma variável chamada
  `$message`.
  */
  let $message = doc.querySelectorAll('textarea');
  /*
  - Selecione o botão de envio do formulário, atribuindo-o à uma variável
  chamada `$button`.
  */
  let $button = doc.querySelectorAll('button');
  /*
  ...
  */
  ...
  function isValidEmail(email) {
        let regex = new RegExp('^[_+\.\\d\\w]+@\\w{2,}\.+(?:\\w{2,})*[\.]*\\w+[\.]*', 'gm');
